...
pacman::p_load(devtools, car, tidyverse, janitor, imputeTS, sjPlot, zoo, compareGroups, forecast, lubridate, gganimate, data.table, CausalImpact, panelView, ggforce, tsModel, astsa, plm, gsynth, Synth, CausalMBSTS, parallel, doParallel, PanelMatch, kableExtra, Amelia, missRanger, install=F)
...
```{r 3a-exp-tray, echo=T, paged.print=TRUE,eval=T}
...
par(mfrow=c(2,2))
by(cons_brechas_gine_a_imp$rem_consulta, cons_brechas_gine_a_imp$id, function(i) { acf(i, lag.max = 12) })
by(cons_brechas_gine_a_imp$rem_consulta_nueva, cons_brechas_gine_a_imp$id, function(i) { acf(i, lag.max = 12) })
by(cons_brechas_gine_a_imp$rem_consulta_control, cons_brechas_gine_a_imp$id, function(i) { acf(i, lag.max = 12) })
by(cons_brechas_gine_a_imp$rem_pertinentes, cons_brechas_gine_a_imp$id, function(i) { acf(i, lag.max = 12) })
by(cons_brechas_gine_a_imp$rem_nsp, cons_brechas_gine_a_imp$id, function(i) { acf(i, lag.max = 12) })
by(cons_brechas_gine_a_imp$rem_altas, cons_brechas_gine_a_imp$id, function(i) { acf(i, lag.max = 12) })
by(cons_brechas_gine_a_imp$entradas, cons_brechas_gine_a_imp$id, function(i) { acf(i, lag.max = 12) })
by(cons_brechas_gine_a_imp$sum_le, cons_brechas_gine_a_imp$id, function(i) { acf(i, lag.max = 12) })
...
collapse::psacf(data.frame(cons_brechas_gine_a_imp, index = c("cod_deis","lev_ano_mes")), rem_consulta + rem_consulta_nueva+ rem_consulta_control+ rem_pertinentes+ rem_nsp+ rem_altas + entradas+ sum_le  ~ cod_deis, ~lev_ano_mes) 
```
</div>

```{r 5a-exp-tray, echo=T, paged.print=TRUE,eval=F}
#https://rdrr.io/cran/panelAR/man/panelAR.html
```

```{r 5b-exp-tray, echo=T, paged.print=TRUE,eval=F}
